...
description: Use whenever the user asks to create, add, build, implement, edit, or polish an IAP, paywall, subscription, pricing, purchase, restore, trial, sale-badge, product-selection, or Remote Config driven view or flow in BaseSwiftUI, including IAP screens under BaseSwiftUI/MT-Screens when present, product display helpers, IAP route presentation, purchase SDK behavior, and Product extension helpers. This is the complete view workflow for IAP views; do not use baseswiftui-swiftui-ui as the primary skill for IAP screens.
---
...
Use this as the complete workflow for BaseSwiftUI IAP and paywall view work. IAP
...
- IAP screens: `BaseSwiftUI/MT-Screens/IAP` when the feature exists.
- IAP integration: existing purchase SDK or service files in `BaseSwiftUI/Libs`, `BaseSwiftUI/Helper`, or `BaseSwiftUI/MT-CleanArchitecture`.
- Product helpers: product-related extensions under `BaseSwiftUI/Exts`.
...
- Search existing helpers and integration files first:
  - product-related files under `BaseSwiftUI/Exts`
  - purchase or IAP files under `BaseSwiftUI/Libs`
  - purchase or IAP files under `BaseSwiftUI/Helper`
  - related domain/service files under `BaseSwiftUI/MT-CleanArchitecture`
...
- Read `BaseSwiftUI/MT-Screens/IAP/README_IAP.md` when it exists and the task changes IAP structure,
...
4. Put reusable IAP wording or product display logic in `BaseSwiftUI/Exts`.
...
- Put reusable product display logic in `BaseSwiftUI/Exts`.
...
- Do not append reusable `extension` blocks to IAP screen files. Put reusable
  Swift extensions in dedicated files under `BaseSwiftUI/Exts`.
