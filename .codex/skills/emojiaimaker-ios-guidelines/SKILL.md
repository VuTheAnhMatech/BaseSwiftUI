---
name: emojiaimaker-ios-guidelines
description: Use when creating, editing, polishing, or reviewing code for the EmojiAIMaker iOS project. This includes SwiftUI screens, Container/MVI flows, Factory dependency injection, Base component reuse, Asset/Color/Font management, or any project-specific architectural and UI rules.
---
# EmojiAIMaker iOS Architecture & Guidelines
...
* **Workspace Integrity**: Always preserve CocoaPods configuration. Build, test, and validate changes exclusively through `EmojiAIMaker.xcworkspace`.
...
* **App Entry**: `EmojiAIMaker/ContentView.swift` (Manages top-level routing: `SplashView -> MainTabbarView`).
* **Feature Screens**: `EmojiAIMaker/MT-Screens/` (Organized by feature folders containing Views and Containers).
* **MVI Containers**: Colocated within their respective feature folders under `MT-Screens/`.
* **Dependency Injection**: `EmojiAIMaker/MT-Factory/` (Centralized Factory registrations).
* **Domain Entities**: `EmojiAIMaker/MT-CleanArchitecture/Domain/Entities/` (App data models, screen/domain entities, static mock data, and sample data such as `MockData`). Use `emojiaimaker-model-organization` for all model placement. Put feature-specific entities in a feature subfolder such as `Domain/Entities/CreateEmojiFromText/`; put truly shared app-wide entities under `Domain/Entities/Shared/`; do not leave unrelated feature models together in flat root files.
* **Core Infrastructure**: `EmojiAIMaker/Base/` (Reusable base components and foundation logic).
* **Global Extensions**: `EmojiAIMaker/Exts/` (Shared Swift system/UI extensions only).
* **Shared Widgets**: `EmojiAIMaker/Widgets/` (Domain-specific reusable UI blocks).
* **Resources**:
  * `EmojiAIMaker/Resources/Assets.xcassets` (Global images, icons, and vector assets).
  * `EmojiAIMaker/Resources/Colors.xcassets` (Color palettes mapped via `Exts`).
  * `EmojiAIMaker/Resources/Fonts/` (Custom font files - Primary: *Plus Jakarta Sans*).
* **Core Libraries**: `EmojiAIMaker/Libs/` (Embedded internal frameworks like `UnionTabView`).

## 3. Base Component & UI Reuse Rules
* **Immutability of Base**: Do not modify files under `EmojiAIMaker/Base` unless explicitly requested by the user or structurally impossible otherwise.
...
* **Shared Color Tokens Only**: Add or modify `Color` extensions only for true reusable app-wide design tokens that already belong to the global token system under `EmojiAIMaker/Exts/`, not for per-screen styling shortcuts.
...
* **Decoupled Adaptations**: When porting patterns or modules from companion projects, strip away all foreign dependencies and streamline the implementation to fit EmojiAIMaker’s lightweight architecture.

## 6. Validation & Verification
Before marking a task as complete, execute a Debug build to guarantee code correctness:
```sh
DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcodebuild -workspace EmojiAIMaker.xcworkspace -scheme EmojiAIMaker -configuration Debug -destination generic/platform=iOS -derivedDataPath /private/tmp/EmojiAIMakerDerivedData CODE_SIGNING_ALLOWED=NO build
```
...
- [ ] Screen/domain models and default/static sample data live under `EmojiAIMaker/MT-CleanArchitecture/Domain/Entities/`, preferably in a feature subfolder when not shared, not in Views, Containers, or Exts.
