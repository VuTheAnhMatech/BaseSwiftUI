...
description: Use when analyzing, implementing, comparing, or updating BaseSwiftUI iOS UI from a Figma design URL or node, including one or multiple screens, normal SwiftUI screens, IAP/paywall screens, widgets, Figma assets, variables, Code Connect, and visual fidelity checks. Supports short analyze, implement, compare, and assets requests and routes automatically to the correct BaseSwiftUI project skills.
---

# BaseSwiftUI Figma UI

Turn a short Figma request into a complete, project-aware design-to-SwiftUI workflow. Treat Figma output as design evidence and adapt it to BaseSwiftUI rather than copying generated reference code.
...
- Normal screen: read `.codex/skills/baseswiftui-swiftui-ui/SKILL.md`.
- IAP, paywall, pricing, trial, purchase, restore, subscription, or sale UI: read `.codex/skills/baseswiftui-iap-flow/SKILL.md` instead.
- WidgetKit, Live Activity, AppIntent, control, or extension-rendered UI: read `.codex/skills/baseswiftui-widget-extension/SKILL.md`.
- Models, screen items, enums, default lists, mock data, or datasource inputs: also read `.codex/skills/baseswiftui-model-organization/SKILL.md`.
- Broad non-view architecture: also read `.codex/skills/baseswiftui-architecture/SKILL.md`.
...
- `BaseSwiftUI/Widgets`
- `BaseSwiftUI/Exts`
- `BaseSwiftUI/Resources/Assets.xcassets`
- `BaseSwiftUI/Resources/Colors.xcassets`
- the nearest folder under `BaseSwiftUI/MT-Screens`
- related entities under `BaseSwiftUI/MT-CleanArchitecture/Domain/Entities`
...
## Implement Idiomatic BaseSwiftUI SwiftUI
...
4. Build through `BaseSwiftUI.xcworkspace` using the validation command from `baseswiftui-ios-guidelines` when code changed.
...
```text
$baseswiftui-figma-ui analyze <figma-node-url>
$baseswiftui-figma-ui implement <figma-node-url>
$baseswiftui-figma-ui compare <figma-node-url>
$baseswiftui-figma-ui assets <figma-node-url>
```
